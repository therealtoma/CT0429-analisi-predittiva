...
mean(mrents[,"rent"])
median(mrents[,"rent"])
quantile(mrents[,"rent"], c(.25,.5,.75))
```
...
```{r}
summary(mrents$rent[mrents$bath ==0])
summary(mrents[mrents$bath == 1, "rent"])
...
```{r}
boxplot(rent ~ bath, data = mrents, col = c("red","orange"), main = "Price per bathroom type") 
```
...
computeSumSquares <- function(bs, y, x){
  if(length(x) != length(y)) stop("lengths are different")
  y_hat <- bs[1] + bs[2] * x
...
for(i in 1:7){
  for(j in 1:9){
    ssqseq[i,j] <- computeSumSquares(c(b0seq[i], b1seq[j]), 
...
## the linea goes through 
mean(cars$speed); mean(cars$dist)
coef(fit)[1] + coef(fit)[2]*mean(cars$speed)
## residuals hav e0 mean
 head(fit$residuals)
 head(cars$dist - fit$fitted.values)
 mean(fit$residuals)
```
