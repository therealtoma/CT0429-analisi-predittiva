...
y_hat <- fitted(fit)
head(y_hat)
## could also have computed directly  
head(coef(fit)[1]+coef(fit)["year"]*autompg$year+coef(fit)["wt"]*autompg$wt)
head(X %*% coef(fit))
```
...
```{r fittedPlot}
par(mfrow=c(1,2), bty="l",pch=16)
plot(mpg~wt, data= autompg, main = expression(paste("MPG = f(wt, ,",bar(year),")")),col="grey70")
nd <- data.frame(wt = seq(1500, 5200, length.out = 100), year = mean(autompg$year))
lines(nd$wt, predict(fit, nd), lwd=2,col=4)
plot(mpg~year, data= autompg, main =expression(paste("MPS = f(year, w = ",tilde(w),")")),col="grey70")
...
legend("topleft",col = c(2, 5), lwd = 1, 
       legend = c(expression(paste(tilde(w),"=1613")),
                  expression(paste(tilde(w),"=2811"))))
```
...
```{r predictFun}
nd <- data.frame(wt = c(1650, 3000, 5000,6000), year = c(72, 75, 82,80))
