...
```{r}
est_sigma <- sum(residuals(fit)^2)/(n-length(coef(fit))) 
est_sigma <- sqrt(est_sigma); est_sigma
summary(fit)$sigma
...
```{r}
cov2cor(est_sigma ^2 * solve(t(X) %*% X))
```
...
alpha = 0.06
cbind(beta_hat  + qt(alpha/2, df = n-3) * se_beta_hat,
      beta_hat  + qt(1-alpha/2, df = n-3) * se_beta_hat)
confint(fit, level = 0.94)
```
...
car::confidenceEllipse(fit, which.coef = c(1,3), levels = 0.94)
abline(v = confint(fit, "(Intercept)", level = 0.94), col = 2)
abline(h = confint(fit, "year", level = 0.94), col = 2)
```
...
points(autompg$wt, y_hat, col = 2)
plot(mpg~year, data= autompg,col="grey50")
points(autompg$year, y_hat, col = "blue")
```
...
nd <- data.frame(wt = seq(1500, 5000, by = 100), 
                 year = 76)
z <- predict(fit, newdata = nd)
lines(nd$wt, z, col = 2, lwd = 3)
title(main = "WT - year = 1976")

plot(mpg~year, data= autompg,col="grey50")
nd <- data.frame(wt = 3000, 
                 year = seq(70, 82, by = 2))
z <- predict(fit, newdata = nd)
lines(nd$year, z, col = 4, lwd = 3)
title(main = "Year - Wt = 3000")
...
nd <- data.frame(wt = c(1650, 3000, 5000), 
                 year = c(72, 75, 82))
cint <- predict(fit, interval = "confidence", 
                newdata = nd, level = 0.94)
cint
x0 <- as.matrix(cbind(rep(1,3), nd))
# se(x0) 
sex0 <- est_sigma * sqrt(diag(x0 %*% solve(t(X) %*% X) %*% t(x0)))
# point estimate 
pest <- x0 %*% beta_hat
cbind(pest + qt(alpha/2, df = n-length(beta_hat)) * sex0,
      pest + qt(1-alpha/2, df = n-length(beta_hat)) * sex0)
```
