...
Il modello ha un p-value molto basso (< 0.05), risultando quindi significativo; inoltre il valore dell' `adjusted R-squared` indica che il modello spiega quasi il **93%** della variabilità.
Notiamo infine che solamente la variabile `numAccess` risulta significativa, `age` ha un p-value troppo alto
...
prima di tutto creiamo il modello `fit2`
```{r}
...
capiamo, attraverso il test `anova`, se il modello `fit2` risulta più significativo rispetto al modello `fit1`
```{r}
anova(fit1, fit2)
```

Confrontando i risultati ottenuti dal test anova, concludiamo che non possiamo rifiutare l'ipotesi nulla \[H_0: \beta_{age} = 0\] contro l'ipotesi alternativa \[H_a: \beta_{age} \not = 0\] concludendo dunque che è possibile rimuovere `age` dal modello.
...
calcolo l'intervallo di predizione:
```{r}
pred <- predict(
  fit2,
  newdata = data.frame(numAccess = c(60, 200)),
...
Notiamo che il secondo intervallo di confidenza (relativo ai 200 accessi) è molto più ampio rispetto al primo: questo perchè all'interno del dataset non sono presenti casi in cui un utente ha eseguito più di 200 accessi.
la stessa cosa vale per il secondo intervallo di predizione.

La differenza tra intervallo di confidenza e intervallo di predizione sta nel fatto che l'intervallo di confidenza rappresenta l'errore attorno al valore medio, l'intervallo di predizione da una misura dell'incertezza attorno al valpre previsto.
...
summary(fitTot)
```
In questo caso si sta stimando che, al crescere del numero di accessi, il numero di richieste diminuisce, cosa che però non è vera. Inoltre la variabile `numRequests` è poco significativa, quindi non possiamo rifiutare l'ipotesi nulla $H_0: \beta_{numRequest} = 0$
