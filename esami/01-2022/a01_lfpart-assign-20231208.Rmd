...
Possiamo notare che il tempo passato dall'utente è influenzato solamente dal numero di accessi (`numAccess`) che sono eseguiti, l'età (`age`) è trascurabile. Nonostante ciò il modello è in grado di spiegare quasi il **93%** della variabilità totale dei dati.
...
Dal risultato del test `anova` notiamo che il modello `fit2` ha un **f-value** molto basso e un **p-value** alto. Possiamo concludere quindi che il modello `fit2` risulta più significativo rispetto al modello con tutte le variabili.

Stiamo cercando di valutare il sistema di ipotesi $H_0: \beta_0 = 0 \ VS \ H_a: \beta_0 \not = 0$, concludendo che non possiamo rifiutare l'ipotesi nulla a favore dell'ipotesi alternativa.
...
Notiamo che il secondo intervallo, in entrambi i casi, risulta molto più ampio rispetto al primo. Questo succede perchè all'interno del dataset non sono presenti casi in cui un utente ha effettuato 200 accessi. Questo implica che il modello deve predirre valori che sono molto più grandi rispetto ai dati che conosce.

Un intervallo di **confidenza** rappresenta l'incertezza attorno al valore medio del numero di accessi, l'intervallo di **predizione** indica l'incertezza attorno i valori effettivi.

Noto, inoltre, che i valori negli intervalli di confidenza sono minori rispetto ai valori negli intervalli di predizione; questo succede perche la media, per definizione, è meno variabile rispetto alle singole osservazioni
...
Notiamo che la variabile `numAccess` è significativa all'interno del modello, mentre `numRequests` no. Questo indica quindi che i fallimenti del sistema risultano molto influenzati dal numero di accessi che vengono fatti.
...
```{r}
print(step(fitTot, direction = 'backward', trace = 0, k = 2)) # AIC
print(step(fitTot, direction = 'backward', trace = 0, k = log(450))) # BIC
```

Nptiamo che BIC ha selezionato un modello con meno parametri. Questo succede perchè BIC tende a penalizzare maggiormente i modelli che hanno molti parametri, specialmente nel caso in cui log(2) risulta tanto maggiore di 2.
