...
summary(fit1)
```
Il t-value risulta molto alto con un p-value molto basso. Il modello è in grado di spiegare quasi il 98% della variabilità dei dati.


```{r}
confint(fit1, level = .98)
```

```{r}
library(car)
linearHypothesis(fit1, "CashFlow = 10")
```
